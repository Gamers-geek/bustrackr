...
export default function SearchBar() {
    ...
    return (
        <div className="relative">
            ...
            {/* Résultats */}
            {filteredStops.length > 0 && showResults && (
                <div
                    className="absolute mt-2 w-full max-h-60 overflow-y-auto rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-lg z-10">
                    {filteredStops.map((stop, idx) => (
                        <a href={`/stop/${stop.code}`}>
                            ...
                            >
                                <div className="font-medium">{stop.name} <span
                                    className="text-zinc-500 dark:text-zinc-400">({stop.city})</span></div>
