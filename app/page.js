                ...
                <div className="grid gap-4 sm:grid-cols-2">
                    {favourites.length > 0 ?
                        favourites.map((favourite, i) => (
                            <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow pb-4.5 pt-4 px-4 space-y-4"
                                 key={i}>
                                <div className="flex items-center justify-between mb-2">
                                    <p className="font-semibold">{favourite.name} <span
                                        className="text-sm text-zinc-500 dark:text-zinc-400">({favourite.city})</span>
                                    </p>
                                </div>
